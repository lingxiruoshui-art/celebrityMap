...
var getConfig = async (db, key, defaultValue = "") => {
...
var setConfig = async (db, key, value) => {
...
async function getRemainingQuota(db) {
  const limit = parseInt(await getConfig(db, "guest_explore_limit", "5"), 10);
  const date = getCSTDate();
  const row = await db.prepare("SELECT count FROM guest_usage WHERE ip = 'GLOBAL_GUEST' AND date = ?").get(date);
  const used = row ? row.count : 0;
  return Math.max(0, limit - used);
}
async function incrementUsage(db) {
  const date = getCSTDate();
  await db.prepare(`
...
async function callAI(c, db, prompt, responseFormat = "text", schema) {
...
async function addRelationship(db, p1, p2, type) {
...
app.post("/admin/config", async (c) => {
  ...
  if (body.gemini_api_key !== void 0) await setConfig(db, "gemini_api_key", body.gemini_api_key);
  if (body.gemini_model_id !== void 0) await setConfig(db, "gemini_model_id", body.gemini_model_id);
  if (body.aliyun_api_key !== void 0) await setConfig(db, "aliyun_api_key", body.aliyun_api_key);
  if (body.aliyun_model_id !== void 0) await setConfig(db, "aliyun_model_id", body.aliyun_model_id);
  if (body.guest_explore_limit !== void 0) await setConfig(db, "guest_explore_limit", String(body.guest_explore_limit));
...
app.get("/metadata", async (c) => {
  const db = await getDb(c);
  const existingPeopleNames = (await db.prepare("SELECT name FROM people").all()).map((p) => p.name).join("\u3001");
  return c.json({
...
app.post("/save-archive", async (c) => {
  ...
  const existing = await db.prepare("SELECT id, biography FROM people WHERE name = ?").get(name);
  ...
  try {
    ...
    if (data.relationships && Array.isArray(data.relationships)) {
      for (const rel of data.relationships) {
        const matched = await db.prepare("SELECT id FROM people WHERE name = ?").get(rel.personName);
...
app.post("/pathfind", async (c) => {
  ...
  const people = await db.prepare("SELECT id, name, raw_relationships FROM people").all();
  const relationships = await db.prepare("SELECT * FROM relationships").all();
  ...
  people.forEach((p) => {
    try {
      const raw2 = JSON.parse(p.raw_relationships || "[]");
      raw2.forEach((r) => {
...
app.get("/archiver/random-pair", async (c) => {
  const db = await getDb(c);
  const count = await db.prepare("SELECT COUNT(*) as count FROM people").get();
  if (count.count < 2) return c.json({ error: "Need at least 2 people in database" }, 400);
  const people = await db.prepare("SELECT name FROM people ORDER BY RANDOM() LIMIT 2").all();
  return c.json({ sourceName: people[0].name, targetName: people[1].name });
});
app.post("/archiver/pick-target", async (c) => {
  const db = await getDb(c);
  const existing = (await db.prepare("SELECT name FROM people").all()).map((p) => p.name);
  ...
  if (unarchivedInPool.length > 0) {
    ...
    const peopleRels = await db.prepare("SELECT raw_relationships FROM people").all();
...
app.post("/archiver/generate-target", async (c) => {
  ...
  const existingNames = (await db.prepare("SELECT name FROM people").all()).map((p) => p.name).join("\u3001");
  try {
...
app.post("/archive-figure", async (c) => {
  const db = await getDb(c);
  const { personName, stream: stream2 } = await c.req.json();
  ...
  if (stream2) {
    ...
    return streamSSE(c, async (stream3) => {
      const send = async (data) => await stream3.writeSSE({ data: JSON.stringify(data) });
      try {
        ...
        const existing = await db.prepare("SELECT id FROM people WHERE name = ?").get(targetName);
        let personId;
        if (existing) {
...
app.post("/save-relationship", async (c) => {
  ...
  const p1 = await db.prepare("SELECT id FROM people WHERE name = ?").get(sourceName);
  const p2 = await db.prepare("SELECT id FROM people WHERE name = ?").get(targetName);
  if (p1 && p2) {
