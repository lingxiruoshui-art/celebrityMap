                ...
                <div className="space-y-4">
                  {path.map((step, i) => (
                    <div key={i} className="flex flex-col items-center">
                      {i > 0 && (
                        <div className="flex flex-col items-center py-2 w-full px-4">
                          <div className="h-4 w-px bg-slate-100"></div>
                          <div className="w-full bg-indigo-50/50 px-4 py-2.5 rounded-xl text-[11px] font-medium text-indigo-600 border border-indigo-100/40 shadow-sm leading-relaxed text-center">
                            {step.type || "历史渊源"}
                          </div>
                          <div className="h-4 w-px bg-slate-100"></div>
                        </div>
                      ...
                      <motion.div 
                        ...
                        className="w-full p-3 bg-white border border-slate-100 rounded-2xl shadow-sm flex items-center justify-between group cursor-pointer hover:border-indigo-400 hover:shadow-indigo-100/50 transition-all"
                        onClick={() => {
                          // Selection is handled in parent via the path itself usually
                        }}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${newArrivals.includes(step.name) ? 'bg-emerald-600 text-white' : 'bg-slate-50 text-indigo-500 group-hover:bg-indigo-600 group-hover:text-white ring-1 ring-slate-200/50'}`}>
                            <User className="w-4 h-4" />
                          </div>
                          <div>
                            <span className="font-black text-slate-800 text-[12px]">{step.name}</span>
                            <p className="text-[9px] text-slate-400 font-medium">{newArrivals.includes(step.name) ? "AI 馆藏同步完成" : "历史跨度关联人物"}</p>
                          </div>
                        </div>
                        {newArrivals.includes(step.name) && (
                          <div className="flex items-center gap-1 px-2 py-0.5 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-md font-black text-[8px] uppercase tracking-tighter">
  ...
  if (isInline) {
    return (
      <div className="flex flex-col w-full">
        {!hideHeader && (
          <div className="p-3 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between cursor-grab active:cursor-grabbing drag-handle hover:bg-slate-100 transition-colors">
            <div className="flex items-center gap-2 pr-2">
              <Zap className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
              <span className="text-[11px] font-bold text-slate-800 whitespace-nowrap">时空关系网络探索</span>
            </div>
            <button 
